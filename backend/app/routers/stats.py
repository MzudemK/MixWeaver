...
async def get_top_tracks(request: Request, limit: int = 10, time_range: str = "medium_term"):
...
async def get_top_artists(request: Request, limit: int = 10, time_range: str = "medium_term"):
    spotify = SpotifyService(request.session)
    if not spotify.handler.get_cached_token():
        raise HTTPException(status_code=401, detail="Not authenticated")
